...
export class ServerModel {
	...
	public claimSegment(bearerToken: string, segmentId: number, cards: BusCard[]): Command[] {
		...
		if (this.commandManager.inDrawingCardState(game.id))
			throw new Error(ErrorMsgs.CANNOT_CLAIM_SEGMENT);
		...
		const claimCommand = this.commandManager.addCommand(
			game.id,
			"claimSegment",
			{ segmentId: segmentId },
			{ cards: cards },
		...
		let newPlayer = this.incrementGameTurn(game);
		let turnCommand = this.commandManager.addCommand(
			game.id,
			"incrementTurn",
			{ playerTurnName: newPlayer },
			{},
		...
		if (routesCompleted.length) {
			returnCommands.push(
				this.commandManager.addCommand(
					...
					"routesCompleted",
					{},
					{ completedRoutes: routesCompleted },
					player.name
				)
			);
		}
	...
	private handleChatCode(
		...
		switch (messageText) {
			...
			case ChatCodes.END_GAME:
				const stats: GameOverStat[] = game.calculateScores();
				game.endGame();
				return this.commandManager.addCommand(game.id, "endGame", { stats }, {}, playerName);
			...
			case ChatCodes.CLAIM_TO_END:
				...
				if (bearerToken) this.claimSegment(bearerToken, 1, mycards);
	...
	selectBusCard(bearerToken: string | undefined, index: number): ICommand[] {
		...
		if (
			card.color === BusColor.wild &&
			index < 5 &&
			this.commandManager.inDrawingCardState(game.id)
		) {
			game.revokePlayerCard(player.username, index);
			throw new Error(ErrorMsgs.CANNOT_DRAW_RAINBOW);
		}
		let turnOver =
			(card.color === BusColor.wild && index < 5) ||
			this.commandManager.inDrawingCardState(game.id);
		let publicData = index < 5 ? { cardColor: card.color } : {};
		let drawCommand = this.commandManager.addCommand(
			game.id,
			"drawBusCard",
			publicData,
			{ cardColor: card.color },
		...
		let newSpreadCommand = this.commandManager.addCommand(
			game.id,
			"updateSpread",
			newSpreadData,
			{},
		...
		if (turnOver) {
			let newPlayer = this.incrementGameTurn(game);
			let incrementCommand = this.commandManager.addCommand(
				game.id,
				"incrementTurn",
				{ playerTurnName: newPlayer },
				{},
