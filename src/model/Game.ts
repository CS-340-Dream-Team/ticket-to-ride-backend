...
export class Game {
	...
	constructor(host: Player, name: string, id: number) {
		...
		for (let i = 0; i < segments_json.length; i++) {
			...
			this.segments.push(
				new Segment(s["id"], s["start"], s["end"], s["length"], s["pair"], s["color"])
			);
		}
	}
	...
	calcLongestRoute(segments: Segment[]) {
		...
		for (const player of this.players) {
			let lengths = [0];
			const playerSegments: Segment[] = segments.filter(segment => segment.owner === player);
			for (const segment of playerSegments) {
				lengths.push(
					this.longestPathForSegment(
						segment.start,
						segment.end,
						segment.length,
						[segment],
